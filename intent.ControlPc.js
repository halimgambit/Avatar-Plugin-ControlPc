...
exports.default = function (state, actions) {
	...
	var match;
	for (var rule in Config.modules.ControlPc.rules) {	 
		match = (0, _helpers.syntax)(state.sentence, Config.modules.ControlPc.rules[rule]); 	
		if (match) break;
	}
	
	if (match) { // vérifié !
		if (state.debug) info('Intent ControlPc, Syntax: true');
		state.isIntent = true;
